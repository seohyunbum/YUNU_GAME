...
// 2026-06-28: 그리드 제작 모호성 수정 — 재료 같은 형제 레시피(용 장비 4종)에서 '재료 넣기'로 고른 의도 레시피 우선(pendingGridRecipeId 필드+set/clear 3곳, workbenchRecipeFromSlots filter+prefer). +5 → 10172.
...
// 2026-07-04: 일리아 사망 = 무조건 차원 밖 이탈로 변경(제자리 재시작 폐지 — 유저가 문 재진입으로 재도전). illiaFight.active||illiaInArena 단일 분기가 leaveCave 로 정리, 일반 핸들러의 wasInIllia 죽은코드 제거. 순 −5 → 9439.
// 2026-07-05: 오버월드 보스/용 텔레그래프(일리아식 범위 예고) — 엔진·필드는 telegraph 리프. main 은 필드 소유(1)+vfx(1)+dragon/predator 컨텍스트 spawnTelegraph(2)+update 호출/맵전환 clear. 순 +4 → 9443.
...
// 2026-07-13: 거너 총기 장전 배선(gunReload 리프) — reload 필드 + startReload arrow 필드 + fireRangedWeapon 게이트/소비 + updateHand tick + KeyV 수동장전 + HUD 탄창 칩 → 9474→9485. 로직은 전부 리프(game/gunReload), main 은 배선만(메서드 수 482 불변).
const MAX_MAIN_LINES = 9486;
