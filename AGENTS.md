...
- 의미 있는 실패·되돌림·보류가 있었으면 → **커밋 전/최종 보고 전 `docs/work-history.md` 에 결과와 이유를 기록**한다. 성공 이력만 남기면 같은 실패가 반복된다.
...
`docs/work-history.md` 는 재발 방지용 작업 일지다. 단순 성공 로그가 아니라, **실패한 시도·되돌린 변경·보류한 접근**을 남기는 문서다.

기록 대상:

- 검증에서 실패한 구현 시도
- 성능이 나빠져 되돌린 최적화
- 커맨드/샌드박스/도구 문제처럼 반복될 수 있는 작업 환경 이슈
- "가능해 보였지만 하지 않기로 한" 접근과 그 이유
...
- 결과
- 실패 또는 보류 이유
- 다음 작업자가 반복하지 말아야 할 판단
- 관련 파일, 테스트, 커밋이 있으면 함께 기록

원칙: 실패 기록은 책임 추궁이 아니라 **다음 시도를 더 똑똑하게 만드는 지도**다. 같은 문제가 다시 보이면 먼저 `docs/work-history.md` 를 확인한다.

## 12. 멀티에이전트 협업 규약 (Codex + Claude Code)

두 에이전트가 같은 repo 를 동시에 작업한다. 같은 파일을 동시에 편집하면 read→edit 사이에 파일이 바뀌어 편집이 무효화되고 작업이 꼬인다(실제 사고는 `docs/work-history.md` 참조). 다음을 지킨다:

- **공유 파일은 한 번에 한 에이전트만.** 특히 `src/main.ts`(지휘자)는 두 에이전트가 동시에 편집하지 않는다.
