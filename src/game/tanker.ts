...
import { ARMOR_VALUE, SHIELD_DEFENSE, toolMaxDurability } from "./items";
...
export function consumeShieldHit(equippedShield: ItemId | null, shieldDurabilityUsed: number): ShieldHitResult {
  ...
  if (used < Math.max(1, toolMaxDurability(equippedShield))) return { equippedShield, shieldDurabilityUsed: used, brokenItem: null }; // 어드민 튜너블(흑요석 방패 내구도) 경유
...
export function tankerHudStatus(equippedShield: ItemId | null, shieldDurabilityUsed: number, ironGuardUntilMs: number, now: number) {
  const shield = equippedShield ? `${Math.max(0, toolMaxDurability(equippedShield) - shieldDurabilityUsed)}/${toolMaxDurability(equippedShield)}` : "없음";
