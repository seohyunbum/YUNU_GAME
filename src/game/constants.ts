...
export const SAVE_VERSION = 16; // 16: 마석 시스템(player.runeSlots·equippedRunes) 추가 — 해금 슬롯 수·장착 마석 유지. 15: 서브퀘스트 상태(player.subquests)
...
export const DRAGON_ARMOR = 85; // 보스 방어 상향 2차: 현재대비 +30% (65→85)
export const DRAGON_BOSS_BAR_DISTANCE = 30; // 보스 HP바 표시 거리(충돌면 기준) — 1이라 밀착해야만 보여 "때려도 잔여 HP 를 모르겠다" 신고. 근접·원거리 전투 거리를 덮게 30(+어그로 중엔 거리 무관, main.updateBossBar)
...
export const MAGIC_AOE_RADIUS = 1.5; // 마법 투사체 착탄 시 소형 범위 피해 반경(주 대상 주변 추가타) — 1.8→1.5 소폭 축소
export const MAGIC_AOE_DAMAGE_FRACTION = 0.5; // 스플래시(주변 추가타)는 주 대상 대비 50%만 — 광역 과보정 완화
...
export const SAMURAI_SKILL_COST = 30; // 사무라이 1스킬 '난도' — 4연격(합 ≈ 공격력 2.8배, 55%→70% 상향 2026-07-04)
export const SAMURAI_SKILL_COOLDOWN = 16;
