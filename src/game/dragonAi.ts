...
const DRAGON_LEASH_RADIUS = 8; // 추격이 끝난 용이 스폰 홈에서 이 거리 이상 벗어나 있으면 복귀 비행
...
export function updateDragons(context: DragonAiContext, delta: number) {
  ...
  for (const dragon of context.dragons()) {
    ...
    } else if (dragon.homePosition) {
      // 리시 복귀 — 추격이 끝난 용은 스폰 지점으로 날아 돌아간다(멀리 도망가면 그 자리에 방치되던 버그 수정).
      const hdx = dragon.homePosition.x - dragon.root.position.x, hdz = dragon.homePosition.z - dragon.root.position.z;
      const hd = Math.hypot(hdx, hdz);
      if (hd > DRAGON_LEASH_RADIUS) {
        const step = (DRAGON_CHASE_SPEED * 0.7 * delta) / Math.max(hd, 0.001);
        dragon.root.position.x = Math.max(-WORLD_SIZE / 2 + 6, Math.min(WORLD_SIZE / 2 - 6, dragon.root.position.x + hdx * step));
        dragon.root.position.z = Math.max(-WORLD_SIZE / 2 + 6, Math.min(WORLD_SIZE / 2 - 6, dragon.root.position.z + hdz * step));
        clampOutOfSafeZones(dragon.root.position);
      }
    }
