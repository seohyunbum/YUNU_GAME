...
const RAW_TUTORIAL_STEPS: readonly TutorialStep[] = [
  ...
  checkQuest("get_grinder", (s) => s.countItem("grinder") > 0, "분쇄기 구하기", "제작대 3×3에서 망치 2개 + 철 6개로 분쇄기를 만드세요. 분쇄기가 있으면 광물을 가루로 만들 수 있어, 철 가루 20개 + 석탄 가루 5개를 모아 대장장이(대장간 NPC)에게 제련대와 교환할 수 있습니다. 마을 상점에서 4200P에 구매할 수도 있습니다.", { experience: 190, items: { coal: 4, iron: 2 }, label: "경험치 190 + 석탄 4개 + 철 2개" }),
  checkQuest("get_smelter", (s) => s.hasSmelter || s.countItem("smelter") > 0, "제련대 구하기", "분쇄기로 철 가루 20개 + 석탄 가루 5개를 만들어 대장장이에게 제련대와 교환하세요. 대장간은 시작 지점 근처의 큰 마을에 있습니다. 흑요석 상자 드랍이나 마을 상점(2600P)으로도 얻을 수 있습니다. 제련대가 있어야 철을 제련할 수 있습니다.", { experience: 200, items: { coal: 6 }, label: "경험치 200 + 석탄 6개" }),
...
function scaleQuestRewards(steps: readonly TutorialStep[]): readonly TutorialStep[] {
  ...
  return steps.map((step, index) => {
    ...
    const preGradFactor = graduationIndex >= 0 && index < graduationIndex ? PRE_GRADUATION_XP_FACTOR : 1;
    const experience = Math.round(step.reward.experience * mult * preGradFactor);
