...
function tuneMaterial(material: THREE.Material | THREE.Material[]) {
  ...
  if (material instanceof THREE.MeshStandardMaterial) {
    ...
    material.color.offsetHSL(0, 0.024, 0.01); // 시네마틱: 채도 절제(필름 룩 — 캔디 아님), 명도 리프트 최소(대비는 CSS grade 가 담당)
    material.flatShading = true;
    material.envMapIntensity = Math.max(material.envMapIntensity, 0.55); // HDRI 은은한 하늘 반사
