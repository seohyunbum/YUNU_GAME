...
import {
  ...
  rollDragonLoot as rollDragonLootItem, rollDragonLootCount, varyPlayerDamage, varyMonsterDamage,
  type ProjectileDamageContext, type ProjectileDamageOptions,
} from "./game/combat";
...
import { createSpirit, createSpiritCollection, equippedSpirit, sanitizeGiftedSpirit, experienceForNextSpiritLevel, gainSpiritExperience, normalizeSpiritCollection, spiritAttackBonus, spiritDefenseBonus, spiritFeedExperience, spiritGradeDef, spiritGradeIndex } from "./game/spirits";
...
import { fortressBossConceptForStage } from "./game/fortressBoss"; import { animateFortressBossModel, createFortressBossModel } from "./game/fortressBossVisuals"; // 시즈 5단계 보스 — 트레일러 소품·idle 애니(엔진·패턴은 fortressSiege 가 구동)
import { spawnFortressMonster, spawnSiegeBossMonster, updateCaveMonsters, type CaveMonsterContext, type FortressSpawnDeps } from "./game/caveMonsters";
...
import { SAMURAI_DASH_PASSTHROUGH_TYPES, useSamuraiPrimarySkill } from "./game/samurai";
...
import { createIlliaFightState, createIlliaCutsceneState, startIlliaFight, resetIlliaFight, updateIlliaFight, startIlliaCutscene, updateIlliaCutscene, finishIlliaCutscene, showIlliaCutsceneOverlay, hideIlliaCutsceneOverlay, ILLIA_P1_KIND, ILLIA_P2_KIND, ILLIA_ENTRY_POS, ILLIA_ARENA_CENTER, clampToIlliaArena, flashIlliaHit, setIlliaTelegraphDifficultyMul, type IlliaContext, type IlliaCutsceneContext } from "./game/illiaBoss"; import { createIlliaModel, createSealStone, createDimensionGateVisual, animateIlliaProps } from "./game/illiaVisuals"; // 최종 보스 일리아 — 패턴 엔진·컷씬·비주얼(leaf)
import { createTelegraphField, updateTelegraphField, spawnFieldTelegraph, clearTelegraphField, telegraphContains, type TelegraphField, type TelegraphFieldVfx } from "./game/telegraph"; // 일리아식 범위 예고 — 오버월드 보스/용·요새 보스 공유 중앙 필드
...
class WildernessGame {
  ...
  private readonly worldSpawnContext: EnvironmentSpawnContext & DroppedItemSpawnContext & GuardSpawnContext & NpcSpawnContext & VillageSpawnContext = {
    addWorldObject: (type, name, root, extra) => this.addWorldObject(type, name, root, extra),
    ...
    trackCaveObject: (id) => { this.caveObjectIds.push(id); },
    isHouseInterior: () => this.locationMode === "house",
  };
  ...
  private readonly objectiveClaimDeps: ObjectiveClaimDeps = { gainExperience: (n) => this.gainExperience(n), addItem: (i, c) => this.addItem(i, c), dropItem: (i, c) => spawnDroppedItem(this.worldSpawnContext, i, c, this.playerPosition.clone()), showMessage: (t) => this.showMessage(t), renderHud: () => this.renderHud(), questExpMultiplier: () => this.difficultyMods.questExp };
  ...
  private lastDamageCountered = false; // 직전 피격이 4차 반격으로 완전 무효화됐는지 — 가드 근접 공격 메시지 중복 방지용
  private readonly projectileDamageContext: ProjectileDamageContext = {
    ...
    recordBossDefeat: (kind) => {
      ...
      if (result.message && result.bossChapter === FINAL_BOSS_CHAPTER) { this.ensureDimensionGate(); let trailer = false; if (this.locationMode === "overworld") for (const gate of this.objectsOfType("dimensionGate")) { trailer = true; showIlliaCutsceneOverlay(this.uiRoot, "차원의 문"); startIlliaCutscene(this.illiaCutscene, "gateOpen", performance.now(), [], gate.root); break; } if (!trailer) this.showMessage("🌌 차원의 문이 열렸습니다! 용의 땅에 나타난 문으로 들어가 최후의 존재와 맞서세요.", { durationSeconds: 10, lockSeconds: 5 }); } // 진짜 엔딩은 일리아 클리어로 이관 — 게이트가 보이는 오버월드에선 10초 개방 트레일러(스킵 가능), 아니면 안내만
    },
    ...
    damagePlayer: (amount, showParticles, deathReason, ignoreArmor) => this.damagePlayer(varyMonsterDamage(amount), showParticles, deathReason, ignoreArmor), // 몬스터 피해 80%~130% 랜덤(반격 등 적 발 피해)
  ...
  private readonly graveTrapContext: GraveTrapContext = {
    ...
    damagePlayer: (amount, showParticles, deathReason) => this.damagePlayer(varyMonsterDamage(amount), showParticles, deathReason), // 좀비 피해 80%~130% 랜덤
    showMessage: (text) => this.showMessage(text),
    renderHud: () => this.renderHud(),
  };
  ...
  private readonly dragonAiContext: DragonAiContext = {
    ...
    monsterChaseSpeedMul: () => this.difficultyMods.monsterChaseSpeed, monsterAttackMul: () => this.difficultyMods.monsterAttack,
    damagePlayer: (a, s, r, attacker) => { const dmg = varyMonsterDamage(a); if (this.tryCounterReflect(attacker, dmg)) return false; return this.damagePlayer(dmg, s, r); }, showMessage: (t) => this.showMessage(t), playTone: (f, d, ty, v) => this.playTone(f, d, ty, v), // 용 피해 80%~130% 랜덤
    spawnTelegraph: (spec, detonateAt, groundY, onDetonate) => spawnFieldTelegraph(this.overworldTelegraphs, this.scene, spec, detonateAt, groundY, onDetonate),
  };
  private readonly overworldTelegraphs: TelegraphField = createTelegraphField(); // 오버월드 보스/용 범위 예고(일리아는 IlliaFightState 자체 관리)
  private readonly telegraphFieldVfx: TelegraphFieldVfx = { scene: this.scene, now: () => performance.now(), groundBurst: (x, z) => spawnGroundShockwave(this.combatEffectContext, new THREE.Vector3(x, this.getGroundHeightAt(x, z) + 0.1, z), 0xff5a1a), playTone: (f, d, ty, v) => this.playTone(f, d, ty, v) };
  ...
  private readonly fieldBossContext: FieldBossContext = {
    locationMode: () => this.locationMode, worldMapId: () => this.currentWorldMapId,
    defeatedFieldBosses: () => partyFilterDefeatedBosses(this.defeatedFieldBosses), // 파티 호스트: 같은 맵 파티원 교집합 — 미처치 파티원 있으면 기처치 보스도 재스폰(공동 사냥)
  ...
  private readonly summonerPetContext: SummonerPetContext = {
    ...
    dropKillSpiritToken: (wild, boss) => this.dropKillSpiritToken(wild, boss), // 펫이 막타 시 정령 소환권은 펫 주인(로컬)에게
    celebratePetLevel: (level) => celebrateLevelUp(this.juiceDeps, level),
    renderHud: () => this.renderHud(),
  };
  private readonly interiorContext: InteriorContext = { scene: this.scene, addWorldObject: (type, name, root, extra) => this.addWorldObject(type, name, root, extra), spawnChest: (position, mineRich) => this.spawnChest(position, mineRich), spawnOre: (ore, position) => this.spawnOre(ore, position), spawnMiner: (position) => spawnMiner(this.worldSpawnContext, position), spawnBlacksmithNpc: (position) => spawnBlacksmithNpc(this.worldSpawnContext, position), randomCavePoint: () => this.randomCavePoint(), rollMineMineral: () => this.rollMineMineral(), spawnFortressMonster: (position, boss) => spawnFortressMonster(this.fortressSpawnDeps, position, boss), trackCaveObjects: (...ids) => this.caveObjectIds.push(...ids), trackHouseObjects: (...ids) => this.houseObjectIds.push(...ids), showMessage: (text) => this.showMessage(text) };
  ...
  private readonly siegeContext: SiegeContext = {
    ...
    showMessage: (text) => this.showMessage(text), renderHud: () => this.renderHud(),
    startBossTrailer: (stage) => { const concept = fortressBossConceptForStage(stage); const prop = createFortressBossModel(concept.key); prop.scale.setScalar(concept.scale); prop.position.set(0, 0, ARENA_CENTER_Z); this.scene.add(prop); showIlliaCutsceneOverlay(this.uiRoot, concept.title); startIlliaCutscene(this.illiaCutscene, "fortressBoss", performance.now(), [prop], null, concept.color); }, // 5단계 보스 강림 트레일러(8s, 스킵 가능) — 소품은 컷씬 종료 시 자동 제거
    spawnSiegeBoss: (stage) => { const boss = spawnSiegeBossMonster(this.fortressSpawnDeps, stage, this.fortressSiege?.baseLevel ?? 1); if (!boss) return null; this.caveObjectIds.push(boss.id); this.enterCombatMood(); this.updateBossBar(); return boss.id; },
    isCutsceneActive: () => this.illiaCutscene.active, isPanelOpen: () => this.currentPanel !== null,
    bossPattern: { now: () => performance.now(), playerX: () => this.playerPosition.x, playerZ: () => this.playerPosition.z, bossX: () => this.objects.get(this.fortressSiege?.aliveIds[0] ?? "")?.root.position.x ?? 0, bossZ: () => this.objects.get(this.fortressSiege?.aliveIds[0] ?? "")?.root.position.z ?? ARENA_CENTER_Z, animateBoss: (t) => { const overlay = this.objects.get(this.fortressSiege?.aliveIds[0] ?? "")?.root.userData.fortressBossOverlay as THREE.Object3D | undefined; if (overlay) animateFortressBossModel(overlay, t); }, spawnBossTelegraph: (spec, damage, label) => { const scaled = { ...spec, delayMs: Math.round(spec.delayMs * this.difficultyMods.bossTelegraph) }; spawnFieldTelegraph(this.overworldTelegraphs, this.scene, scaled, performance.now() + scaled.delayMs, this.getGroundHeightAt(scaled.x, scaled.z), () => { if (telegraphContains(scaled, this.playerPosition.x, this.playerPosition.z)) { this.enterCombatMood(); this.damagePlayer(varyMonsterDamage(Math.round(damage * this.difficultyMods.monsterAttack)), true, `${label}에 맞아 체력이 모두 떨어졌습니다.`); } }); }, showMessage: (text) => this.showMessage(text), playTone: (f, d, ty, v) => this.playTone(f, d, ty, v) }, // 텔레그래프 예고시간·피격 데미지에 난이도 배율 반영(일리아·용과 동일 정책)
  };

  private readonly guardAiContext: GuardAiContext = { guards: () => this.objectsOfTypes(["villageKnight", "villageArcher", "villageMage", "villageGolem"]), playerPosition: this.playerPosition, getGroundHeightAt: (x, z) => this.getGroundHeightAt(x, z), refreshSpatialObject: (object) => this.refreshSpatialObject(object), runWalkCycle: (object, delta, speed) => this.animateWalkCycle(object, delta, speed), monsterChaseSpeedMul: () => this.difficultyMods.monsterChaseSpeed, damagePlayer: (amount, showParticles, reason, attacker) => { this.enterCombatMood(); const dmg = varyMonsterDamage(amount); if (this.tryCounterReflect(attacker, dmg)) { this.lastDamageCountered = true; return false; } this.lastDamageCountered = false; return this.damagePlayer(dmg, showParticles, reason); }, playHandAction: () => this.playHandAction(), showMessage: (text) => this.showMessage(text), renderHud: () => this.renderHud(), getLastDamage: () => ({ blocked: this.lastDamageBlocked, taken: this.lastDamageTaken, countered: this.lastDamageCountered }), keepOutOfBuildings: (position) => keepOutOfBuildings(position, this.objectsNear(position, 7)), fireProjectile: (fx, fy, fz, tx, tz, dmg, kind) => spawnGuardProjectile(this.guardProjectiles, this.guardProjectileContext, new THREE.Vector3(fx, fy, fz), new THREE.Vector3(tx, this.getGroundHeightAt(tx, tz), tz), dmg, kind) };
  private readonly guardProjectiles: GuardProjectile[] = [];
  private readonly guardProjectileContext: GuardProjectileContext = { add: (m) => this.scene.add(m), remove: (m) => this.scene.remove(m), playerPosition: this.playerPosition, damagePlayer: (a, s, r) => this.damagePlayer(varyMonsterDamage(a), s, r), impact: (p, kind) => spawnProjectileImpact(this.combatEffectContext, p, kind === "rock" ? "arrow" : kind) }; // 경비 투사체 피해 80%~130% 랜덤

  private readonly predatorAiContext: PredatorAiContext = { locationMode: () => this.locationMode, isPanelOpen: () => this.currentPanel !== null, playerPosition: this.playerPosition, activeRegions: () => this.activeRegions, predators: () => this.objectsOfType("wildPredator"), predatorAggroRange: (kind) => predatorAggroRangeFor(kind), predatorStrikeRange: (kind) => predatorStrikeRangeFor(kind), predatorStats: (kind, monsterId) => predatorBaseStats(kind, monsterId), monsterChaseSpeedMul: () => this.difficultyMods.monsterChaseSpeed, getGroundHeightAt: (x, z) => this.getGroundHeightAt(x, z), refreshSpatialObject: (object) => this.refreshSpatialObject(object), animateWalkCycle: (object, delta, speed) => this.animateWalkCycle(object, delta, speed), damagePlayer: (amount, showParticles, reason, attacker) => { this.enterCombatMood(); const dmg = varyMonsterDamage(amount); if (this.tryCounterReflect(attacker, dmg)) return false; return this.damagePlayer(dmg, showParticles, reason); }, effects: () => this.combatEffectContext, showMessage: (text) => this.showMessage(text), spawnTelegraph: (spec, detonateAt, groundY, onDetonate) => spawnFieldTelegraph(this.overworldTelegraphs, this.scene, spec, detonateAt, groundY, onDetonate) };
  ...
  private setupRenderer() {
    ...
    this.renderer.toneMappingExposure = 0.98; // 시네마틱 무디 — 노출 절제(대비·비네팅은 CSS, 색조는 timeOfDay). 과노출 마진 확보.
  ...
  private updateTimeOfDay(delta: number) {
    ...
    for (const key in this.homeSupplyCooldowns) if (this.homeSupplyCooldowns[key] > 0) this.homeSupplyCooldowns[key] = Math.max(0, this.homeSupplyCooldowns[key] - delta); if (this.sharedSupplyCd > 0) this.sharedSupplyCd = Math.max(0, this.sharedSupplyCd - delta); // 게스트 공유 보급 쿨타임도 로컬 감쇠(동결값 과차단 방지)
  ...
  private seedOverworld() {
    ...
    for (let i = 0; i < Math.round(1144 * (mapDef.treeScale ?? 1)); i += 1) spawnTree(this.worldSpawnContext, Math.random() < 0.78 ? "smallTree" : "bigTree", this.randomGroundPoint());
    for (const point of [
      ...
      spawnTree(this.worldSpawnContext, "smallTree", point);
    }
    spawnTree(this.worldSpawnContext, "bigTree", new THREE.Vector3(28, this.getGroundHeightAt(28, 34), 34));
    spawnTree(this.worldSpawnContext, "bigTree", new THREE.Vector3(-32, this.getGroundHeightAt(-32, 30), 30));
    ...
    for (const waterZone of this.activeWaterZones) spawnWaterBody(this.worldSpawnContext, waterZone.center.clone(), this.waterZoneRadius(waterZone), waterZone.name);
    this.spawnTrain(0.1);
    for (let i = 0; i < 6; i += 1) this.spawnChest(this.randomGroundPoint(), false, rollChestTier());
    for (let i = 0; i < Math.round(3 * (mapDef.caveScale ?? 1)); i += 1) spawnCave(this.worldSpawnContext, this.randomGroundPoint());
  ...
  private dropItemFromSlot(slot: Slot | null | undefined) {
    ...
    if (partyGuestDropIntercept(item, 1, position.x, position.z)) this.appendPartyLedger(item, -1); else spawnDroppedItem(this.worldSpawnContext, item, 1, position); // 파티 양도=비가역 기록(복제 차단). 솔로 드랍은 미기록 → 불러오기로 복구.
    this.playHandAction();
  ...
  private update(delta: number) {
    if (!this.gameStarted) {
      ...
      if (!this.introPlayed) { this.introPlayed = true; if (!navigator.webdriver) { showIlliaCutsceneOverlay(this.uiRoot, "야생 마을", true); startIlliaCutscene(this.illiaCutscene, "intro", performance.now(), []); } } // 부팅 1회 인트로 트레일러(자동화 환경은 스킵 — E2E/픽셀검사가 타이틀을 즉시 봄)
      if (this.illiaCutscene.active) { this.handGroup.visible = false; updateIlliaCutscene(this.illiaCutscene, this.illiaCutsceneContext); } else this.updateTitleCamera(); // 인트로 중엔 컷씬이 카메라 소유
    ...
    if (this.locationMode === "cave") { if (this.fortressSiege?.active) { updateSiege(this.fortressSiege, this.siegeContext, delta); this.updateBossBar(); } updateCaveMonsters(this.caveMonsterContext, delta); } // 동굴 몬스터·보스 추격 + 요새 디펜스 웨이브 진행 + 보스 체력바(HTML diff 라 저렴)
    ...
    updateDragons(this.dragonAiContext, delta); updateTelegraphField(this.overworldTelegraphs, this.telegraphFieldVfx, this.locationMode === "overworld" || this.fortressSiege?.active === true, this.currentPanel !== null); if (this.illiaCutscene.active) { this.handGroup.visible = false; updateIlliaCutscene(this.illiaCutscene, this.illiaCutsceneContext); } else if (this.illiaFight.active) { updateIlliaFight(this.illiaFight, this.illiaContext, delta); this.updateBossBar(); } if (this.locationMode === "overworld") for (const gate of this.objectsOfType("dimensionGate")) if (gate.root !== this.illiaCutscene.anchor) animateIlliaProps(gate.root, this.worldTimeSeconds); // 일리아: 컷씬 중엔 전투 정지(카메라 연출), 전투 중 매 프레임 패턴 틱+보스바(HTML diff 라 저렴). 개방 트레일러 중인 게이트는 컷씬이 변형 소유
  ...
  private readonly classSkillHandlers: Record<PlayerClassId, () => void> = {
    ...
    samurai: () => useSamuraiPrimarySkill(this.secondSkillContext, (skill) => this.trySpendSkill(skill.name, skill.manaCost, skill.cooldown, "primary")), // 난도 — 로직은 game/samurai 리프, 여기는 배선만
  ...
  private readonly fourthSkillContext: ThirdSkillContext = {
    ...
    trySpend: (skill: SecondSkillDef) => this.trySpendSkill(skill.name, skill.manaCost, skill.cooldown, "fourth"), castImpact: () => spawnSkillCastImpact(this.combatEffectContext, this.playerClass, true, "fourth"), // G 궁극 시그니처(슬롯별 이펙트 차별화)
  ...
  private readonly thirdSkillContext: ThirdSkillContext = {
    ...this.secondSkillContext,
    trySpend: (skill: SecondSkillDef) => this.trySpendSkill(skill.name, skill.manaCost, skill.cooldown, "third"), castImpact: () => spawnSkillCastImpact(this.combatEffectContext, this.playerClass, this.jobTier >= 4, "third"), // F 시그니처(슬롯별 이펙트 차별화)
    nearbyCombatTargets: (radius) => { const targets: WorldObject[] = []; for (const object of this.objectsNear(this.playerPosition, radius + 4)) if (this.isCombatTarget(object) && Math.hypot(object.root.position.x - this.playerPosition.x, object.root.position.z - this.playerPosition.z) <= radius + (object.collisionRadius ?? 0)) targets.push(object); return targets; },
    heal: (amount) => { if (this.health < this.maxHealth) { this.health = Math.min(this.maxHealth, this.health + amount); spawnHealEffect(this.combatEffectContext, this.playerPosition); this.renderHud(); } },
    spiritStorm: (radius) => spawnSpiritStorm(this.combatEffectContext, this.playerPosition, radius),
  };
  ...
  private clampPlayerHorizontalPosition(position: THREE.Vector3) {
    ...
    if (this.locationMode === "cave") {
      ...
      } else if (this.illiaInArena) {
        clampToIlliaArena(position); // 일리아 차원(오버월드 밖 -1250) — 사각 동굴 복도 클램프가 아레나 밖으로 끌어당기던 것 차단, 원형 결계로 일원화
      } else {
  ...
  private updateEnvironmentHazards(delta: number) {
    if (partyWorldGuestActive() || partyGuestSuppressLocalBosses() || this.locationMode !== "overworld" || !this.isPointInLava(this.playerPosition)) { // 파티 게스트 — 용암 드래곤도 호스트 권위(로컬 스폰이 스윕과 충돌·"각자 잡음" 방지)
  ...
  private checkStepEvents(moved: number) {
    ...
    while (this.caveStepBank >= CAVE_STEP_INTERVAL) {
      ...
      if (pity || Math.random() < 0.2 * (getWorldMapById(this.currentWorldMapId).caveScale ?? 1)) {
        spawnCave(this.worldSpawnContext, this.pointNearPlayer(26, 44));
  ...
  private resolveCollisions(position: THREE.Vector3, playerHeight = this.currentPlayerHeight(), passThroughCreatures = false) {
    for (let pass = 0; pass < 3; pass += 1) {
      let changed = false;
      for (const object of this.objectsNear(position, 7)) {
        if (!object.collidable || (passThroughCreatures && SAMURAI_DASH_PASSTHROUGH_TYPES.has(object.type))) continue; // 도약 관통 — 몬스터·보스 등 생명체는 통과(건물·지형·설치물은 그대로 막힘)
  ...
  private claimHomeSupply() {
    if (partyGuestStorageActive()) { if (this.sharedSupplyCd > 0) { this.showMessage(`아직 보급이 차지 않았습니다. ${homeSupplyReadyLabel(this.sharedSupplyCd)}.`); return; } sendSupplyClaim(); this.showMessage("보급함을 열었습니다 — 호스트가 공유 창고에 넣는 중…"); return; } // 게스트: 선차단 후 호스트에 요청. 실수령 목록은 supplyResult 회신으로 표시
  ...
  private leaveCave() {
    ...
    this.fortressSiege = null; clearTelegraphField(this.overworldTelegraphs, this.scene); resetIlliaFight(this.illiaFight, this.scene); this.illiaInArena = false; if (this.illiaCutscene.active) { this.illiaCutscene.active = false; for (const prop of this.illiaCutscene.props) this.scene.remove(prop); this.illiaCutscene.props = []; this.illiaCutscene.anchor = null; hideIlliaCutsceneOverlay(this.uiRoot); this.handGroup.visible = true; } // 어떤 경로로 동굴을 나가든 siege·일리아 전투·컷씬 해제(파티 summonTo 가 컷씬 중 leaveCave 를 호출해도 카메라 강탈/보스 오버월드 누출이 없도록 onFinish 미발화 수동 정리)
  ...
  private enterFortressSiege(target: WorldObject) {
    ...
    this.showMessage(startStage > 1 ? `🏰 몬스터 요새 재입성 — ${startStage}단계부터 이어서 시작! (이 맵 최고 클리어 단계) 중앙을 사수하세요. (사망/중도 퇴장해도 받은 보상은 유지)` : "🏰 몬스터 요새 입성 — 1단계 도전 시작! 중앙을 사수하세요. 단계를 클리어할수록 전직의서·보상이 커집니다. (사망/중도 퇴장해도 받은 보상은 유지)");
    this.renderHud();
  }
  ...
  private attackAnimal(target: WorldObject) {
    const damage = varyPlayerDamage(this.currentDamage()); // 80%~200% 범위 랜덤(우편향)
  ...
  private attackPredator(target: WorldObject) {
    applyMeleePredatorAttack(this.projectileDamageContext, target, varyPlayerDamage(this.currentDamage())); // 80%~200% 랜덤
  }

  private attackDragon(target: WorldObject) {
    applyMeleeDragonAttack(this.projectileDamageContext, target, varyPlayerDamage(this.currentDamage())); // 80%~200% 랜덤
  }
  ...
  private dragonCounterAttack(target: WorldObject) {
    ...
    if (claw) {
      ...
      this.damagePlayer(varyMonsterDamage(stats.clawDamage * this.difficultyMods.monsterAttack), true, `${stats.name}의 손톱 공격을 받아 체력이 모두 떨어졌습니다.`); // 80%~130% 랜덤 × 난이도 공격배율
    ...
    this.damagePlayer(varyMonsterDamage(stats.fireDamage * this.difficultyMods.monsterAttack), true, `${stats.name}의 원거리 공격을 받아 체력이 모두 떨어졌습니다.`); // 80%~130% 랜덤 × 난이도 공격배율
  ...
  private grantExperienceForTarget(target: WorldObject, creditQuest = true) {
    ...
    if (target.fieldBossId && !this.defeatedFieldBosses.includes(target.fieldBossId)) {
      this.defeatedFieldBosses.push(target.fieldBossId); partyNotifyFieldBossDefeat(target.fieldBossId, target.name ?? "필드 보스"); // 게스트 단독 처치도 파티 전체에 기록 전파(호스트 릴레이)
  ...
  private attackJammini(target: WorldObject) {
    ...
    const damage = varyPlayerDamage(this.currentDamage()); // 80%~200% 랜덤
  ...
  private attackKnight(target: WorldObject) { if (partyGuestAttackIntercept(target, this.currentDamage(), "melee")) return; // 파티 게스트 — 호스트가 판정
    const attack = varyPlayerDamage(this.currentDamage()); // 80%~200% 랜덤
  ...
  private fireRangedWeapon(item: ItemId) {
    ...
    const projectile: CombatProjectile = {
      ...
      damage: varyPlayerDamage(this.currentRangedDamage(item)), // 80%~200% 랜덤(발사 시 1회 굴림)
  ...
  private applyProjectileDamage(target: WorldObject, attackPower: number, kind: CombatProjectile["kind"], options?: ProjectileDamageOptions) {
    applyProjectileDamageWithContext(this.projectileDamageContext, target, attackPower, kind, options);
  }
  ...
  private damagePlayer(amount: number, showParticles = true, deathReason = "체력이 모두 떨어졌습니다.", ignoreArmor = false) {
    if (this.illiaCutscene.active) return false; // 컷씬(트레일러) 중 무적 — 카메라 강탈 중 사망/사망화면 진입 방지
    ...
    if (this.health <= 0) {
      if (this.illiaFight.active || this.illiaInArena) {
        // 일리아 결전 사망 — 아이템 미드랍. 차원 밖(문 앞)으로 밀려나며, 유저가 차원의 문에 다시 들어가 재도전한다.
        // leaveCave() 가 illiaFight 리셋·보스/졸개 제거·illiaInArena 해제·문 앞(caveReturnPosition) 이동·오버월드 복귀를 모두 처리.
        this.health = this.maxHealth; this.hunger = HUNGER_MAX; this.hungerTimer = 0;
        this.leaveCave();
        this.showMessage("💀 일리아에게 쓰러져 차원 밖으로 밀려났습니다. 아이템은 잃지 않았습니다 — 차원의 문에 다시 들어가 재도전하세요! (패턴을 기억하세요)", { durationSeconds: 8, lockSeconds: 3 });
        flashDeathBanner("💀 일리아에게 쓰러졌습니다");
        this.renderHud();
        return true;
      }
      if (this.fortressSiege?.active) {
      ...
      if (this.locationMode === "cave") {
        ...
        this.clearCaveObjects();
        this.setOverworldAtmosphere();
      } // 일리아 차원 사망은 위 illiaInArena 분기가 먼저 처리(아이템 미손실·문 밖 이탈)하므로 여기 도달 시엔 일반 동굴
      if (this.locationMode === "house") {
        this.locationMode = "overworld";
        this.currentHouseKind = "home";
        this.currentHouseOwned = false;
  ...
  private dropInventoryOnDeath(position: THREE.Vector3) {
    ...
    for (const slot of this.allStorageSlots()) {
      ...
      spawnDroppedItem(this.worldSpawnContext, slot.item, slot.count, position.clone().add(scatter));
      slot.item = null;
      slot.count = 0;
      slot.durabilityUsed = undefined;
    }
  ...
  private restoreSaveData(sourceSave: SavedGame | PartialSavedGame) {
    ...
    reconcilePartyLedger(localStorage, this.currentCharacterId, save.player.partyLedgerEpoch ?? 0, {
      add: (item, count) => { if (item.startsWith("__spirit__:")) { const gifted = sanitizeGiftedSpirit(JSON.parse(item.slice(11))); if (gifted && !this.spirits.owned.some((o) => o.id === gifted.id)) this.spirits.owned.push(gifted); return true; } return this.addItem(item, count); }, // 정령 선물 수신 재적용(세이브 롤백에도 받은 정령 유지)
      remove: (item, count) => { if (item.startsWith("__spirit__:")) { const gifted = sanitizeGiftedSpirit(JSON.parse(item.slice(11))); if (gifted) { const idx = this.spirits.owned.findIndex((o) => o.id === gifted.id); if (idx >= 0) { if (this.spirits.equippedId === gifted.id) this.spirits.equippedId = null; this.spirits.owned.splice(idx, 1); } } return; } this.removeItem(item, count); }, // 정령 선물 양도 재적용(롤백 복제 차단)
      onLeftover: (item, count) => { if (!transferSlot({ item, count }, this.homeStorage)) spawnDroppedItem(this.worldSpawnContext, item, count, this.playerPosition.clone()); }, // 인벤 가득 → 집창고, 그것도 가득 → 발밑(유실 0)
    });
  ...
  private restoreWorldObject(savedObject: SavedObject) {
    ...
    if (savedObject.type === "smallTree" || savedObject.type === "bigTree") object = spawnTree(this.worldSpawnContext, savedObject.type, position);
    ...
    if (savedObject.type === "miner") object = spawnMiner(this.worldSpawnContext, position);
    ...
    if (savedObject.type === "villageSellShop") object = spawnVillageSellShop(this.worldSpawnContext, position, villageId);
    ...
    if (savedObject.type === "villageFence") {
      ...
      object = spawnVillageFence(this.worldSpawnContext, position, radius, villageId);
  ...
  private updateBossBar() {
    ...
    if (!dragon && this.fortressSiege?.bossPhase === "fight") dragon = this.objects.get(this.fortressSiege.aliveIds[0] ?? "") ?? null; // 요새 5단계 보스 — cave 라 직접 조회(분모=siegeBossMaxHp)
    if (!dragon) { if (!this.bossBarEl.classList.contains("hidden")) { this.bossBarEl.classList.add("hidden"); this.bossBarEl.innerHTML = ""; } return; } // 이미 숨김이면 no-op — 요새에서 매 프레임 호출돼도 DOM 무접촉
    const stats = this.bossStats(dragon.bossKind);
    const maxHp = dragon.siegeBossMaxHp ?? Math.round((dragon.bossKind === ILLIA_P1_KIND || dragon.bossKind === ILLIA_P2_KIND ? bal(dragon.bossKind === ILLIA_P1_KIND ? "illia_p1_hp" : "illia_p2_hp", stats.maxHp) : (dragon.bossKind ?? "dragon") === "dragon" ? bal("dragon_hp", stats.maxHp) : stats.maxHp) * this.difficultyMods.monsterHp); // 보스 hp 도 난이도로 스폰 시 ×배율 → 체력바 분모도 동일 배율(일리아는 튜너블, 요새 보스는 스폰 시 고정값)
    ...
    const html = `
      <div class="boss-title">${dragon.siegeBossMaxHp ? dragon.name ?? "요새의 보스" : stats.name}${!dragon.siegeBossMaxHp && !isBossUnlocked(dragon.bossKind ?? "dragon", this.bossChapter) ? " · 봉인됨" : ""}</div>
  ...
  private teleportToWorldMap(mapId: string, force = false) {
    ...
    this.locationMode = "overworld"; this.clearWorld(); clearTelegraphField(this.overworldTelegraphs, this.scene); const worldState = this.worldStates[map.id]; // 이전 맵의 진행 중 범위 예고 잔재 제거(맵 전환은 오버월드→오버월드라 update 가드가 안 잡음)
  ...
  private spawnVillage(position: THREE.Vector3, houseCount = 5, special = false) {
    ...
    spawnVillageFence(this.worldSpawnContext, position.clone(), ringRadius + 7, villageId);
    ...
    spawnVillageSellShop(this.worldSpawnContext, position.clone().add(new THREE.Vector3(ringRadius * 0.58, 0, -ringRadius * 0.46)), villageId);
