...
class WildernessGame {
  ...
  private dropItemFromSlot(slot: Slot | null | undefined) {
    ...
    if (!partyGuestDropIntercept(item, 1, position.x, position.z)) this.spawnDroppedItem(item, 1, position); // 파티 게스트는 호스트 월드에 생성 요청(공유 → 누구나 줍기)
    this.playHandAction();
