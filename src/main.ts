...
class WildernessGame {
  ...
  private update(delta: number) {
    ...
    updateDragons(this.dragonAiContext, delta); if (this.illiaCutscene.active) { this.handGroup.visible = false; updateIlliaCutscene(this.illiaCutscene, this.illiaCutsceneContext); } else if (this.illiaFight.active) { updateIlliaFight(this.illiaFight, this.illiaContext, delta); this.updateBossBar(); } if (this.locationMode === "overworld") for (const gate of this.objectsOfType("dimensionGate")) if (gate.root !== this.illiaCutscene.anchor) animateIlliaProps(gate.root, this.worldTimeSeconds); // 일리아: 컷씬 중엔 전투 정지(카메라 연출), 전투 중 매 프레임 패턴 틱+보스바(HTML diff 라 저렴). 개방 트레일러 중인 게이트는 컷씬이 변형 소유
