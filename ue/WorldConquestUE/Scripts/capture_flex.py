# 유연 캡처 — 임의 UE 플래그로 창 띄우고 최대화(고 DPI 재현) 후 실해상도 grab.
# 사용: python capture_flex.py <out_name> <settle_sec> <flag1> <flag2> ...
#   예) python capture_flex.py fsel_max 12 -WCShowSelect
#       python capture_flex.py mapsel_max 14 -WCTurns=0 -WCSelect=hanseong
# 항상 최대화(WC_MAXIMIZE 무관) — 짤림은 최대화(스케일~1.5)에서 재현되므로.
...
time.sleep(SETTLE)
user32.ShowWindow(hwnd, 3)   # SW_MAXIMIZE — 고 DPI 재현
time.sleep(1.2)
force_fg(hwnd)
