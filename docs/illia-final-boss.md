...
## 아레나 결계·이펙트·컷씬 강화 (2026-07-05)

**원형 결계 (반경 16, 몬스터 요새 폭 수준)**
- 뒤로 빠지며 원거리로 때리는 카이팅이 너무 쉬웠음 → `clampToIlliaArena`(illiaBoss)로 전투 중 매 프레임 반경 16 클램프 + 반투명 결계벽·상단 룬 링 시각화. 출구 포탈은 결계 안(r≈14.5)이라 이탈은 항상 가능.
- **아레나를 오버월드 밖 `ILLIA_CENTER_Z=-1250`으로 이전**(WORLD_SIZE 1800=±900): 기존 -875(요새와 공유)는 실제 오버월드 안이라 용의 땅 나무·지형이 아레나에 노출됐음. cave 모드 지형높이=0이라 안전. ★함정: `clampPlayerHorizontalPosition`의 cave 복도 클램프가 아레나 밖으로 견인 → illiaInArena 분기로 원형 결계에 일원화. 심연 돔+보이드 바닥으로 완전 차폐.
- 패턴 사거리 재조정: 속박 링 3.6~13.5(안전=발밑/가장자리 띠), 부채꼴 r 17(측면 회피 강제), 강림 후속 링 10.8~16.4(인-아웃), 절망의 비 = 결계 원판 균등 낙하.

**텔레그래프·폭발 이펙트**
- 수축 경고 링(×1.55→경계, 남은 시간 직관화) + 채움 색 램프(어두운 핏빛→밝은 진홍, `lerpColors` 무할당) + 최종 0.35s 스트로브 점멸.
- 폭발: 형태별 빛기둥(가산 원통, 0.45s — 직선은 경로 전체 슬래브) + 볼리당 저역 폭음 + 피격 시 붉은 비네트 섬광(`flashIlliaHit`) + 저음 타격음.
