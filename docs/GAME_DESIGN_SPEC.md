...
> **문서 버전**: v1.7 (2026-07-17) — 개정 이력: v1.0 최초 작성 · v1.1 §5.6 밸런스 편집 도구 · v1.2 §2.7 시네마틱 + §2.8 초빙(가챠) 신설 · v1.3 §4.4 결정론 개정 승인(명명 RNG 스트림+PCG32) + 일기토(§2.6) 구현 확정 — 관련 종속·미확정 항목 해소 · v1.4 §7 즉시 묶음 — §0.3 커밋·검증 규율(6·7), §4.2 SaveSystem 계약, §4.4 정수 산술 표준, §5.5 스키마 버전·콘텐츠 id 생애주기, §8 세이브 roundtrip·해석기 방어, §9 갭 리스크 6행 · v1.5 미확정 결정 반영 — 페이크아웃 미채택·T3캡 5 확정·AI영상 배제·선물 pity 전진 허용·천장/비용 현재값 유지+패널 조정(§5.6 편집 그룹 5종) · v1.6 §2.3.1 내정 상세 확정 — 태수 스탯 연동(정치→생산·건설, 매력→민심·징병, 지력→기술)·민심 승수·세율 3단계·인구 징병·반란·시설 6종(시장/농지/항구/학당/병영/성벽); 병행 세션 tech 안 통합; data schema_version 2 · v1.7 §2.8.0 무장 획득 3경로 — start_characters(시작 배치)+등용(recruit, 사신 매력·금·결정적 판정) 구현 · v1.8 §2.3.2 수치제 경제 + 파견(dispatch) 모델 — 상업·농업 수치화(거점별 max)+개발/징병군수/탐색 파견(POL·LDR·INT 의존)+태수 자동개발; 시장·농지 시설 대체; search:{faction} 스트림; data schema_version 3
...
6. **커밋 전 검증 필수**: 커밋 전 `verify`(= `dotnet build -warnaserror && dotnet test`) 녹색을 확인한다. 깨진 상태로 커밋하지 않으며, 깨지면 직전 녹색 커밋으로 되돌린다. CI 미비 시에도 로컬 규율로 발효하며 `.github/workflows/verify.yml` 로 기계 강제한다.
7. **스키마 변경 원자성**: 세이브·데이터 스키마의 형태(필드 추가·의미 변경·제거)를 바꾸면 **버전 상향 + 마이그레이션/호환 판정 + 왕복(roundtrip) 테스트**를 **같은 커밋 단위**로 넣는다. 커밋은 pathspec 으로 대상을 한정하고(같은 repo 의 다른 프로젝트 오염 금지 — 통합 경로 미정, `AGENTS.md`), 동작 보존 '이동'과 동작 변경 '개선'을 한 커밋에 섞지 않는다.
...
| 자원 | 용도 | 획득 |
|---|---|---|
| **금(Gold)** | 병력 유지비, 시설 건설, 등용, 외교 | 영지 상업 개발 수치(§2.3.2) + 항구 시설 |
| **식량(Food)** | 병력 유지, 원정 보급 (부족 시 사기 급락) | 영지 농업 개발 수치(§2.3.2) + 항구 시설 |
...
- 영지별 시설 슬롯(기본 4개): 병영 / 학당 / 성벽 / 항구(교역 금·식량). ※ 상업·농업은 시설이 아니라 수치제 개발(§2.3.2) — 시장·농지 시설 폐지.
- 각 시설은 레벨 1~3, 건설·업그레이드에 금 + 소요 턴


#### 2.3.1 내정 상세 설계 (v1.6 구현 확정 — InternalAffairsManager 소유)

**모든 계수는 `game_rules.json:internal_affairs` (§5.4), 산술은 정수 (§4.4).** `per_100_stat` 계수의 실효 = 스탯 × 계수 / 100 (내림). 건설 즉시 완성(소요 턴은 후속 확장).

**태수(Governor) 파견** — 영지당 1명, 소속(CharacterOwners) 무장만. **한 무장 = 한 보직** [MUST]: 부대·함대 지휘관과 태수는 겸직 불가(양방향 검증, AssignCommander 에도 소속 검증 추가), 영지 상실·반란 시 자동 해임.

| 태수 스탯 | 효과 | 기본 계수 |
|---|---|---|
| 정치(POL) | 금·식량 생산 보너스 % | 금 15 / 식량 10 per 100 |
| 정치(POL) | 시설 건설비 할인 (상한 30%) | 20 per 100 |
| 매력(CHA) | 민심 회복 /턴 | 5 per 100 |
| 매력(CHA) | 징병비 할인 (병영과 합산, 상한 40%) | 15 per 100 |
| 지력(INT) | 기술 포인트 /턴 | 10 per 100 |

**민심(PublicOrder)** — 영지별 0~100 (기본 70). 생산 승수 = `65 + PO×50/100` % → 기본 민심에서 정확히 100%, 100이면 115%, 0이면 65%.
- 드리프트(수입 페이즈) = 세율 드리프트 + 태수 매력 회복. 징병 시 인구 1000당 −2.
- 점령 전이: 전투 40 / 무혈 60 / 반란 후 50.
- **반란** [MUST]: 민심 < 20 영지는 턴당 25%(만분율 2500) 확률로 중립 독립 — `world_events` 스트림 판정(결정적), `ProvinceRebelled` 이벤트. 진압 = 재점령.

**세율** — 세력 단위 3단계 (`tax <단계>`): low 금70%·민심+3 / normal 금100%·민심0 / high 금130%·민심−5. 식량은 세율 무관.

**수입 산식(단일 소스 = `PreviewIncome`)**: `기본 × (100+시설%+태수POL%) /100 × 민심승수 /100 × 세율(금만) /100` — 각 단계 정수 내림.

**인구** — 영지별 가변(기본 = 정의값). 징병 시 병종별 `pop_cost` 소모(창병 1·공성추 3·함선 5~25·**그림자 병사 0**), 부족 시 거부. 성장 = `인구 × 100(만분율) × PO/100 /10000` /턴, 상한 = 정의 인구 × 150%.

**기술** — 포인트 = Σ학당(레벨×30) + Σ태수 INT×10/100 /턴. 레벨업 비용 = 현재Lv × 200, 상한 Lv5. **징병 시 병종 `tech_required` > 세력 기술Lv 이면 거부** (§2.5 해금 게이트). (다른 세션 병행 tech 안 — academy 직접가산 — 은 이 누적 방식으로 통합·대체함.)

**시설 4종** (v1.8 — 시장·농지는 §2.3.2 수치제로 대체) — 경제: 항구(교역 금 +18%/Lv·식량 +10%/Lv) / 기술: 학당(기술점) / 군사: 병영(징병 할인 5%/Lv)·성벽(수비 방어 +10%/Lv, `CombatManager.ResolveAuto` 지형 방어에 합산). 슬롯 4개 중 택. 효과 필드는 `FacilityDef` 범용(§5 데이터 주도).

**콘솔 명령(§2.2 확장)**: `province [영지]` / `governor <영지> <무장>` / `dismiss <영지>` / `tax <단계>` / `chars`. UE5 클라이언트도 API(GameSessionHost)로 governor/dismiss/tax 사용, StateSnapshot 에 민심·인구·태수·세율·기술점 additive 노출.

**AI 연동(§4.2 AIController)**: 전선 행동 전에 ①보직 없는 소속 무장을 정치 순으로 태수 파견(생산 높은 영지 우선) ②여유 자금(비용 3배) 시 항구→학당→병영→성벽 우선순위로 턴당 1건 건설 ③남은 유휴 무장을 상업·농업 개발에 파견(§2.3.2) — 전부 ordinal, 난수 0.

#### 2.3.2 수치제 경제 + 파견(dispatch) 모델 (v1.8 구현 확정 — §2.3.1 시장·농지 대체)

**모든 계수는 `game_rules.json:economy` / `:search` (§5.4), 산술은 정수 (§4.4).** 이 절은 §2.3.1 의 "시장(금)·농지(식량)" 시설 경제를 **수치제 + 파견 모델**로 대체한다(항구·학당·병영·성벽 시설은 유지). 설계 의도: "내정·징병·등용·탐색 모두 무장을 **파견**해 그 무장의 능력치에 성과가 좌우된다".

**상업(Commerce)·농업(Agriculture) — 수치제 개발** [MUST]: 각 육상 거점은 `commerce`/`agriculture` 개발 수치를 가지며, **거점 특성별 상한**(`commerce_max`/`agriculture_max`, `world_map.json`)까지 성장한다. 상한은 지형·항구 기반으로 파생(도시 상업↑·평야 농업↑·항구 상업 ×1.2). 시작값 = 상한 × `dev_start_pct_of_max`(40%). ProvinceState 에 additive 저장(null=시작값 해석).

**수입 산식(단일 소스 = `PreviewIncome`, §2.3.1 대체)**: `(기본생산 floor + 상업×gold_per_commerce_permil/1000) × (100+시설%+태수POL%)/100 × 민심승수/100 × 세율/100` (금). 식량은 농업×`food_per_agriculture_permil/1000` + 기본생산 floor, 세율 무관. 기본생산(`base_production`)은 무개발 하한일 뿐, 수입의 주 동력은 개발 수치.

**파견 모델 — 한 무장 = 턴당 1행동** [MUST]: 개발·징병(군수)·탐색은 소속 무장 1명을 파견하고 그 무장은 `ActedCharacterIds`(세력별, 수입 페이즈 리셋, additive 저장)에 소진 기록된다. 등용(recruit)은 사신 매력이 성공률을 좌우하되 세력 캡(`recruit_general.max_per_turn`)으로 제한하는 반복형 외교라 소진 대상에서 제외.

| 행동 | 파견 무장 | 능력치 의존 | 산식/효과 |
|---|---|---|---|
| **개발(Develop)** | 임의 유휴 무장 | 정치(POL) | 증가량 = `dev_base_gain` + POL×`dev_pol_gain_per_100`/100, 상한 clamp. 비용 `dev_cost_gold`. |
| **태수 자동개발** | 태수 | 정치(POL) | 수입 페이즈마다 상업·농업 += POL×`governor_dev_gain_per_100_pol`/100 (상한까지). |
| **징병 군수(muster)** | 선택적 군수 무장 | 통솔(LDR) | 징병비 추가 할인 = min(LDR×`muster_ldr_discount_per_100`/100, `muster_discount_max_pct`). 병영+태수 매력 할인과 별도 가산. |
| **등용(recruit)** | 사신 | 매력(CHA) | §2.8 (기존). |
| **탐색(search)** | 사신 | 지력(INT) | 성공률 = `search.base_chance_permyriad` + INT×`envoy_int_permyriad_per_100`/100, [min,max] clamp. 성공 시 금 `search.gold_reward` 획득. `search:{faction_id}` 스트림(신설, §4.4)·세이브스컴 방지·세력 캡 `search.max_per_turn`. |

**RNG 스트림 신설**: `search:{faction_id}` — 세력별, 탐색 1회 = 난수 1소비(등용 `recruit:{faction_id}` 와 동형). §4.4 명명 스트림 집합에 편입.

**콘솔/API 명령**: `develop <영지> <commerce|agriculture> <무장>` / `search <사신무장>` / `recruit <영지> <병종> <수> [군수무장]`. StateSnapshot 의 ProvinceOwnershipView 에 commerce·commerce_max·agriculture·agriculture_max additive 노출.

> **밸런스 관측 (배치 시뮬 100판, 정보성 [SHOULD])**: 수치제 경제는 세력 공통 규칙이라 *상대적* 균형을 바꾸지 않는다. 확장 지도(육상 42거점)에서 최고 승률 **58%(어벤져스)** — DoD 70% [SHOULD] 게이트 통과. (구 18거점 소형 맵에서는 특정 세력 100% 편중이 나왔으나 지도 확장으로 해소된 소형 맵 아티팩트였다.) 다만 여전히 전 판이 200턴 판정승 교착 — *정복 종결*(턴 상한 전 통일) 미달성은 지도/전투의 별개 부채(§9)로, 경제와 무관하며 §5.6 밸런스 패널·전투 튜닝 안건.
...
> Core 랜덤은 **단일 마스터 시드에서 결정적으로 파생되는 명명(named) 스트림 집합**만 허용한다. v1 스트림: `combat` / `world_events` / `summon:{faction_id}`(세력별, §2.8.5) / `recruit:{faction_id}`(세력별 등용, §2.8.0) / `search:{faction_id}`(세력별 탐색, §2.3.2) / `cinematic`. 스트림 간 소비 격리 [MUST]. 스트림 신설·구현체 변경은 스펙 개정 항목. 구현체는 **상태 직렬화 가능 PRNG(PCG32 권장)** — System.Random 은 상태 직렬화 불가로 세이브→로드 스트림 연속성이 성립하지 않는다.
...
| 판정 | **워크호스. 채택** | **기각** ("T2 촬영본 mp4 굽기"만 허용) | **배제 확정** (2026-07-16 — 별도 과금/구독 불가 방침). 월 구독형 영상 생성 도구 미사용 | **게임 립(rip) 에셋 다운로드 금지 [MUST]**. 모델 확보 불가 캐릭터는 '실루엣+이펙트 중심'(전투는 화면 밖 문법의 확장) no-model 템플릿 변형으로 T2 성립 |
...
> - **T3 하드캡 = 5편 확정** — ①공동 승리 엔딩 ②동맹 체결 ③온주참화웅 ④5★ 초빙 프레임 ⑤적벽.
> - **AI 영상 생성 배제 확정** — 별도 과금/구독 불가 방침. T3 는 UE5 Sequencer + T2 촬영본만(§2.7.9).
>
> ⚠ 미확정 — 사용자 결정 필요:
> 1. **체감 파라미터 기본값 승인**: 스킵 홀드 1.5초 / 전투당 풀버전 상한 2 등 — 전부 config 외부화, Phase 2 부자 실플레이 관찰로 튜닝.
> 2. **메인 메뉴 union 갤러리 조기 도입 여부**: 기본 = Phase 5 유보(신규 영속 표면 방지). 가족 핫시트 특성상 조기 요구 가능.
...
#### 2.8.0 무장 획득 3경로 (v1.7 구현 확정) — 시작 배치·등용·초빙

무장이 세력에 소속되는 길은 셋. 소속 장부는 `CharacterOwners`(charId→factionId) 단일 진실원(§2.8.4 이중 장부 금지).

1. **시작 배치(start)** — `factions.json`: `leader_character_id` + `start_characters[]`. 게임 시작 시 세력이 데리고 출발하는 무장. 현재 리더 1명 + 세력별 0~1명(촉=제갈량·관우, 위=조조·곽가, 오=손권·태사자, 조선=이순신·장영실, 칼데아=세이버·마슈, 어벤져스=아이언맨·헐크). 나머지는 재야. **`start_characters` 는 데이터 필드라 몇 명으로 시작할지는 JSON 편집만으로 조정** (검증: 존재·리더 중복·세력 간 중복 소속 금지, §5.5).
2. **등용(recruit)** — `RecruitmentSystem`: 재야(recruit 채널·미소속) 무장을 **금 + 사신 매력**으로 지목 영입. 특정 대상을 노린다는 점에서 초빙과 다르다. 성공률 = `base + 사신매력×계수 − 대상rarity×페널티` (정수 만분율, [min,max] clamp — `game_rules.recruit_general`). 비용 = `base + rarity×cost_per_rarity`, **실패해도 소모**(사신 경비 → 무한 재시도 억제). 판정은 `recruit:{factionId}` 명명 스트림(§4.4) — 리로드해도 결과 불변(세이브스컴 불가). 턴당 시도 상한. 콘솔 `recruits`(후보·성공률 공시)·`enlist <대상> <사신>`, UE5 는 API `enlist`.
3. **초빙(summon)** — `SummonSystem`: 천명 가챠(아래 §2.8.1~). 대상 미지정 확률 추첨.

등용·초빙 풀은 모두 `CharacterOwners` 미포함에서 파생 — 시작 배치·타 경로로 소속된 무장은 자동 제외.
...
- **진실 신호 [MUST]**: 문 색은 실제 최고 rarity 와 항상 일치한다. **페이크아웃 미채택 확정**(2026-07-16 — 아동 기대-배신 방지) → `fakeout_permyriad` 0 고정이 정본. 데이터 토글은 스키마에 남기되(향후 재검토 여지) 값 0. 리빌 순서·연출 난수(`Hash(pull_id)`)는 유지.
...
> - **페이크아웃 미채택 확정** — `fakeout_permyriad` 0 고정(진실 신호). 아동 기대-배신 방지.
> - **천장·비용 초기값 = 현재 수치 유지** (hard pity 30 / soft 20+600 / 단발 100·10연 900) — **밸런스 패널(§5.6)에서 조정 가능** [MUST]. Phase 2 경제 시뮬로 재튜닝.
> - **선물 초빙장 pity 전진 허용** — `advances_recipient_pity` true. 발행자·수신자 양측 턴 캡 + `max_pending: 3` 로 퍼널링 상한 유지.
>
> ⚠ 미확정 — 사용자 결정 필요:
> 1. **전용 등장 컷씬 v1 명단 3본**: 후보 예 — 관우·이순신·세이버. §5.7 레지스트리 id 를 Phase 0 에서 합의 필요.
> 2. **메타 컬렉션(캠페인 간 영구 로스터) 처분**: 완전 기각 vs Phase 5 이후 v2 백로그 보존 — 2회차 캠페인 동기가 필요해지는 시점에 재부상할 주제. 기본 = v2 백로그.
...
| `SaveSystem` | 게임 상태 직렬화/역직렬화 (모든 세이브 읽기의 단일 진입점) | `Save(slot)`, `Load(slot)`. **[MUST] 계약** — ①`Load` 는 **새 `GameState` 객체를 생성**한다('새 게임 초기화 후 되채움' 금지 → 필드 리셋 함정 원천 차단, `required`/`init` 로 누락을 컴파일 타임 노출) ②마이그레이션 = 단일 `SaveVersion` 상수 + 전 필드 nullable `SaveDto`→`Normalize` 단일 정규화 경로(버전별 체인 금지), **미래 버전만 명시 거부**, `migratedFromVersion` 기록 ③세이브 내 컬렉션의 삭제된 정의 id 참조는 **건별 스킵+카운트+고지**(정의=fail-fast §5.5 와 대칭, 세이브=fail-soft) ④저장은 **페이즈 경계에서만** 허용, `CurrentTurn`·`CurrentPhase` 기록 ⑤**atomic write**(임시 파일 완성 후 교체, 실패 시 기존 파일 무손상). 상세 = Phase 1 `docs/designs/save-system-design.md` |
...
- **게임플레이 판정 산술은 정수/고정소수점만** [MUST]: 데미지·성장·상성·확률 등 결과에 영향을 주는 계산에 부동소수(float/double) 금지. 배율은 정수 스케일로 표현(예: ×1.5 → `150/100`, `growth_rate` 1.2 → `120`), 나눗셈 반올림 규칙(내림/은행가)을 공식마다 명세. **근거**: §7 Phase 3 오라클(콘솔 C# = UE5 C++ 동일 결과)은 언어·컴파일러 간 부동소수 연산이 비트 일치하지 않아 float 로는 구조적으로 깨진다. 세이브 JSON 숫자도 언어중립(정수 또는 고정소수 문자열). **data JSON 밸런스 수치의 정수 스케일 전환은 스키마가 굳기 전 우선 처리**(§5 데이터 개정 + DataLoader·Domain·테스트 동반 — dotnet 환경에서 수행)
...
**데이터 스키마 버전**: `data/config/game_rules.json` 에 `schema_version`(정수) 앵커 필드를 둔다. DataLoader 는 필드 부재·미래 버전을 §5.5 포맷으로 명시 거부한다. (8개 파일 공통 헤더·과거 버전 마이그레이션은 git 공동 버전·§5.5 하드 실패로 현재는 중복 — 파괴적 스키마 개정이 처음 필요한 시점에 재평가.)

**콘텐츠 id 생애주기 [MUST]**: id 는 세이브·진행도의 유일한 앵커다(§2.7 컷씬 id-set·§2.8 정의 참조). 따라서:
- 배포된(세이브에 기록된 적 있는) id 는 **영구 불변** — 표기 변경은 `name_ko` 등 표시 필드만. **id 개명 금지**.
- 삭제한 id 는 **영구 결번(tombstone)** — `data/config/retired_ids.json` 에 등재, DataLoader 가 신규 데이터의 재사용을 검출하면 기동 실패.
- 불가피한 개명은 `aliases`(구 id→신 id) 맵으로 선언, `SaveSystem.Normalize`(§4.2) 가 로드 시 치환(로더가 순환·중복 검사).
- **근거**: id 개명은 세이브에서 삭제+신규로 보여 캐릭터가 조용히 증발하고(스킵 카운트조차 없음), id 재사용은 구세이브를 엉뚱한 정의에 동화시킨다.
...
- 편집 그룹 5종: ①`game_rules` 전역 상수(상성 배율·세율·상륙 디버프·게이지 충전량 등) ②캐릭터 스탯·성장률 ③병종 스탯(육상/해상) ④영지 생산량 ⑤**초빙 경제·확률·천장**(`game_rules.summon` 수입·비용, `rate_tables` 확률·soft/hard pity, `banners` 비용) — 천장·비용 수치는 코드 하드코딩 없이 이 패널에서 조정 [MUST].
...
// ── data/summon/summon_reveals.json (Presentation 해석 전용 — Core 는 읽지 않는다 [MUST]) ──
{
  ...
  "tease": { "gate_color_by_max_rarity": { "3": "blue", "4": "purple", "5": "gold" },  // ★4/★5 시각 분리 — near-miss 제거
             ...
             "fakeout_permyriad": 0,             // 미채택 확정(2026-07-16) — 0 고정이 정본. 활성 시에도 상향 전용 [MUST]
...
// ── data/config/game_rules.json 확장 ──
{
  "summon": {
    ...
    "gift_summon": { "enabled": true,
                     ...
                     "max_pending": 3,
                     "advances_recipient_pity": true },    // 2026-07-16 허용 확정 — 양측 턴 캡 + max_pending 3 으로 퍼널링 상한
...
- **v1.2 증분 (§2.7·§2.8)**:
  - 작업: 등급 스팅 5종 + 컷씬 BGM 덕킹/전환 + 붓글씨 폰트(라이선스 확인) + 문 열림·천장 만개 SFX + 이연 T3(③④⑤) 제작(UE5 Sequencer·T2 촬영본만 — AI 영상 생성 배제 확정) + 정식 트레일러(티저 30초 + 본편 90초, 워터마크 자동 번인)
...
- **세이브 왕복(roundtrip) 3종** [Phase 1 DoD]: ①전 필드 비기본값 '풍부 상태' 픽스처(2인 협동 상태 — 동맹·시야 공유·자원지원 잔여·공동 승리 플래그·현재 턴 액터 포함)로 serialize→load→serialize 정규화 비교(기본값 픽스처면 snapshot/restore 한쪽 누락이 안 드러남) ②**프로세스 재기동 포함 E2E**(정적 캐시·싱글턴이 만드는 가짜 그린 차단) 또는 'N턴 진행→저장→재기동→M턴 == 무중단 N+M턴' 리플레이 동등성 ③세이브 버그 수정 시 그 사고를 설명 메시지 박은 명명 assert 로 같은 테스트에 누적. 구버전 세이브 픽스처는 `tests/fixtures/saves/v1/` 동결.
- **해석기 no-op 방어** [MUST]: SkillSystem 등 데이터 해석기는 미지·미구현 타입을 조용히 무시하지 않고 예외를 던진다(§5.5 '조용한 스킵 리젝' 의 런타임 확장). Phase 2 콘텐츠 게이트가 전 스킬을 실제 해석기로 합성 전투에서 실행해 no-op(상태 변화 0)이면 실패(§2.8.7·§5.7).
- **시드 리플레이 골든**: §4.4 명명 스트림별 상태 다이제스트를 git 골든과 대조, 불일치 시 '--update 후 같은 커밋' 처방. 생성기=검사기 단일 코드 경로(둘이 갈라질 수 없음).
...
| 세이브 호환 깨짐 | 세이브에 스키마 버전 필드, 로드 시 **미래 버전만 거부**. 마이그레이션 = 단일 `Normalize` 경로(§4.2). 세이브 시스템 사고 이력은 `docs/save-system-history.md`(Phase 1 개설) 에 누적 — 건드리기 전 필독 계약 |
...
| 선물 초빙장의 pity 가속 (아빠 재화로 아들 천장 전진) | `advances_recipient_pity` true (허용 확정) — 발행자·수신자 **양측 턴 캡** + `max_pending: 3` 으로 퍼널링 상한. 협동 기능으로 수용, 남용 관측 시 캡 하향 |
...
| 산술 결정론 붕괴 (float 밸런스로 Phase 3 오라클 C#=C++ 비트 불일치) | 게임플레이 산술 정수/고정소수 [MUST] (§4.4) — data JSON 정수 스케일 전환을 스키마 확정 전 우선 처리(dotnet 환경) |
| 콘텐츠 id 개명·재사용 (세이브 앵커 붕괴 — 캐릭터 증발·오동화) | id 영구 불변·삭제 결번(`retired_ids.json`)·`aliases` 맵 [MUST] (§5.5) |
| .NET 8 LTS EOL 2026-11-10 (Phase 2~5 진행 중 도래) | `global.json` 핀 + 차기 LTS(.NET 10) 승급을 Phase 2 게이트 리뷰 안건에. 핀 갱신은 verify 녹색 + 골든 리플레이 무변화 단독 커밋 |
| 빌드·데이터 리비전 추적 불가 (세이브가 어느 빌드 산출인지 모름) | 마일스톤 git tag + 세이브·텔레메트리에 `buildVersion`·`dataRevision` 스탬프. 롤백 시 자동저장 링의 마지막 호환 세이브 안내 |
| gitignore 로컬 파일(오버라이드·텔레메트리·자동저장)의 스키마 진화 사각 | 로컬 전용 파일에도 버전 필드 + 정책 3분법(정의=fail-fast / 세이브=fail-soft+고지 / 로컬=fail-soft+반드시 고지). `AGENTS.md` 로컬 파일 정책 |
| orphan 브랜치 통합 경로 미정 (master=YUNU_GAME 과 공통 조상 없음) | 통합 방식(별도 repo 분리 vs 영구 브랜치)을 `AGENTS.md` 에 확정. CI·커밋 pathspec 으로 교차 오염 차단 |
