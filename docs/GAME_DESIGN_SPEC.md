...
**시설 6종** — 경제: 시장(금)·농지(식량)·항구(교역 금+식량) / 기술: 학당(기술점) / 군사: 병영(징병 할인 5%/Lv)·성벽(수비 방어 +10%/Lv, `CombatManager.ResolveAuto` 지형 방어에 합산). 슬롯 4개 중 택. 효과 필드는 `FacilityDef` 범용(§5 데이터 주도).
...
**AI 연동(§4.2 AIController)**: 전선 행동 전에 ①보직 없는 소속 무장을 정치 순으로 태수 파견(생산 높은 영지 우선) ②여유 자금(비용 3배) 시 시장→농지→항구→학당→병영→성벽 우선순위로 턴당 1건 건설 — 전부 ordinal, 난수 0.
