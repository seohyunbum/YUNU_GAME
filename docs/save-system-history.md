...
- **eat_meat 달성-미수령 상태는 로드 시 유실**(2026-07-04 리뷰 확정 minor, 수용 결정) — eat_meat 는 TUTORIAL_STEPS 미소속 컨텍스트 퀘스트라 achievedStepIds 라치가 없고 ateMeat 시그널은 세이브 미포함. "고기 먹음(1/1, Q 대기)" 상태로 저장→로드하면 0/1 회귀. 손실=고기 1개 재섭취(수초), 보상 유실 없음. 대가로 타 슬롯/새 게임 자동완료 누수 제거가 더 큼. 완전 차단하려면 컨텍스트 퀘스트 라치 또는 ateMeat 세이브 필드화.
